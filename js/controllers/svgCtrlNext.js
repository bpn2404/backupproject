'use strict';
app.controller('svgCtrl', ['$scope', '$rootScope', '$timeout', '$http', '$location', 'ModelPerson', 'Base64Service', 'cardsScreenService', function ($scope, $rootScope, $timeout, $http, $location, ModelPerson, Base64Service, cardsScreenService) {
...
*/
        ...
        hiveY = clippingMaskY + maximumImageDisplayHeight + 50;
        //THE TOP MODEL NAME 
        topHeadlineSvg = mainSvg.text(14, hiveY, ['NEXT']).attr({
            id: 'cardHeadline',
            fontName: 'helvetica',
            fontSize: '60px',
            fill: textColor
    ...
    $scope.drawDetails = function (resetImage) {
        ...
        function onInitializationImageLoaded() {
            ...
            previousGroupX = mainImageGroupX;
            previousGroupY = mainImageGroupY;
    ...
    $scope.updateModelDisplayDetails = function () {
        ...
        for (var i = 0; i < length; i++) {
            ...
            if (counter === 1) {
                ...
                counter++;
                detailsTextSvg.select(selectP1 + iPlusOne + ')').attr({
                    fill: textColor, //'rgb(97, 99, 110)', //same as #61636E
                    fontName: 'helvetica'
                });
            } else if (counter === 3) {
                counter = 1;
        ...
        detailsY = hiveY + bbox.height + detailsSpaceBetweenName;
        detailsTextSvg.attr({
            x: detailsX,
            y: detailsY
        });
    ...
    $scope.drawModelDetails = function () {
        ...
        detailsX = 0;
        detailsY = hiveY + modelNameSvg.node.clientHeight + detailsSpaceBetweenName;
        details = $scope.getDetailsArray();
    ...
    var previousGroupX, previousGroupY;
    $scope.setRotationCenterXAndY = function () {


        var difX, difY;        

        //Get image group position, if the position changed check the direction and calculate the difference to the last known position, then add or substract accordingly
        /*if (mainImageGroupX !== previousGroupX || mainImageGroupY !== previousGroupY) {
            
            if (mainImageGroupX !== previousGroupX) {
                if (!(mainImageGroupX < 0) || (previousGroupX < 0)) {
                    if (mainImageGroupX > previousGroupX) {
                        difX = mainImageGroupX - previousGroupX;
                        centerX = centerX - difX;
                    } else if (mainImageGroupX < previousGroupX) {
                        difX = previousGroupX - mainImageGroupX;
                        centerX = centerX + difX;
                    }
                }
            ...
            if (mainImageGroupY !== previousGroupY) {
                if (mainImageGroupY > previousGroupY) {
                    difY = mainImageGroupY - previousGroupY;
                    centerY = centerY - difY;
                } else if (mainImageGroupY < previousGroupY) {
                    difY = previousGroupY - mainImageGroupY;
                    centerY = centerY + difY;
                }
        ...
        //OLD WAY OF SETTING CENTRE
        var bx = mainImage.getBBox();
        centerX = bx.cx;
        centerY = bx.cy;
    ...
    $scope.reset = function () {
        ...
        previousGroupX = undefined;
        previousGroupY = undefined;
        dotX = undefined;
        dotY = undefined;
